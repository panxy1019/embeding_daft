...
ORM_FIELD_TYPES = {
    ...
    "short_text": DatabaseTextType(length=CM_AHVN.get("ukf.text.short", 255)),
    "medium_text": DatabaseTextType(length=CM_AHVN.get("ukf.text.medium", 2047)),
    "long_text": DatabaseTextType(length=CM_AHVN.get("ukf.text.long", 65535)),
...
class ORMUKFAdapter(BaseUKFAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        fields_sig = ",".join(sorted(self.fields))
        cache_key = f"{self.name}:{md5hash(fields_sig)}"
