...
from sqlalchemy import Column, Index, select, func, delete
...
class DatabaseCache(BaseCache):
    ...
    def _has(self, key: int) -> bool:
        ...
        result = self._db.orm_execute(stmt, readonly=True).to_list(row_fmt="tuple")
        if len(result) != 1:
    ...
    def _get(self, key: int, default: Any = ...) -> Dict[str, Any]:
        ...
        result = self._db.orm_execute(stmt, readonly=True).to_list(row_fmt="dict")
        if len(result) < 1:
            return default
        if len(result) > 1:
