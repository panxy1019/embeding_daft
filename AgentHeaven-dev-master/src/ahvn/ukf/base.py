...
from ..utils.basic.serialize_utils import dumps_json, loads_json, deserialize_func
from ..utils.basic.debug_utils import error_str
from ..utils.basic.config_utils import CM_AHVN, dget, dset, dunset, dsetdef
...
class BaseUKF(BaseModel):
    ...
    **System**: Extensible fields for application-specific data and runtime statistics
    ...
    normalized_synonyms: UKFSetType = Field(
        default_factory=set,
        description=(
            "A set of normalized strings that used to avoid repeated normalization. " "Note that it may contain normalized forms of fields other than synonyms."
        ),
    )
    ...
    processing_status: UKFShortTextType = Field(
        default="normal",
        description=("The processing status of this knowledge. Options: 'normal', 'upserting', 'inserting', 'removing'."),
    )
    ...
    set_fields: ClassVar[Tuple[str]] = ("tags", "synonyms", "normalized_synonyms", "related", "auths")
    ...
    @field_serializer("normalized_synonyms")
    def serialize_normalized_synonyms(self, normalized_synonyms: Set[str]) -> List[str]:
        return sorted(list(normalized_synonyms))
    ...
    def serialize_triggers(self, triggers: Dict[str, Callable]) -> Dict[str, Dict]:
        ...
        for name, trigger in triggers.items():
            if isinstance(trigger, dict) and ("capsule_version" in trigger) and ("layers" in trigger):
                serialized[name] = trigger
            elif callable(trigger):
                serialized[name] = Capsule.from_func(trigger).to_dict()
            else:
                serialized[name] = trigger
        return serialized
    ...
    def validate_triggers(cls, value):
        ...
        if isinstance(value, dict):
            return {k: cls._deserialize_callable_mapping(v) for k, v in value.items()}
        return value
    ...
    def serialize_content_composers(self, composers: Dict[str, Callable]) -> Dict[str, Dict]:
        ...
        for name, composer in composers.items():
            if isinstance(composer, dict) and ("capsule_version" in composer) and ("layers" in composer):
                serialized[name] = composer
            elif callable(composer):
                serialized[name] = Capsule.from_func(composer).to_dict()
            else:
                serialized[name] = composer
        return serialized
    ...
    @classmethod
    def _deserialize_callable_mapping(cls, value: Any):
        if callable(value):
            return value
        if not isinstance(value, Dict):
            return value

        if ("capsule_version" in value) and ("layers" in value):
            from ..utils.capsule import Capsule

            return Capsule._restore_callable(value)

        if ("code" in value) or ("hex_dumps" in value):
    ...
    def tools(self) -> List[str]:
        """Access the list of tools associated with this knowledge item.

        This is a convenience property mapping to 'tools' key in content_resources.
        """
        tools = self.content_resources.get("tools", [])
    ...
    def is_processing(self) -> bool:
        """Return True when the item is in processing.

        Returns:
            bool: True when the item is in processing.
        """
        return self.processing_status in ("upserting", "inserting", "removing")
    ...
    def set_processing_status(self, status: str):
        ...
        if status not in ("upserting", "inserting", "removing"):
            raise ValueError(f"Invalid processing status: {status}")
        self.processing_status = status

    def unset_processing_status(self):
    ...
    _VALID_AUTHORITIES: ClassVar[Tuple[str, ...]] = ("RW", "R")
    _ADMIN_GROUP: ClassVar[str] = "GRP_admin"
    _PUBLIC_GROUP: ClassVar[str] = "GRP_public"
    ...
    def auth(
        ...
        """Initialize or update auth permissions on this UKF object (chainable).

        This method sets up permissions for the UKF. When reset=True (default),
        it clears existing auths and initializes with fresh permissions.
        All UKFs will have "GRP_admin:RW" by default. Additional permissions
        are added based on user_id and roles. If neither user_id nor roles are
        provided, "GRP_public:RW" is added to make the UKF publicly accessible.

        Args:
            user_id: Optional user identifier. If provided and not "admin",
                adds "USER_{user_id}:RW" to auths.
            roles: Optional list of role names. Each role adds "GRP_{role}:RW".
                Note: "admin" role is automatically included.
            reset: If True (default), clears existing auths before adding new ones.
                If False, adds to existing auths without clearing.

        Returns:
            Self for method chaining.

        Examples:
            >>> ukf.auth()  # GRP_admin:RW, GRP_public:RW
            >>> ukf.auth(user_id="alice")  # GRP_admin:RW, USER_alice:RW
            >>> ukf.auth(user_id="alice", roles=["analyst"])
            # GRP_admin:RW, USER_alice:RW, GRP_analyst:RW
            >>> ukf.auth(user_id="bob").signed()  # Chainable with other methods
        """
        auth_tags: Set[str] = set() if reset else set(self.auths)

        # Always add admin group with RW authority
        auth_tags.add(f"[{self._ADMIN_GROUP}:RW]")

        # Add public group if no user or roles specified
        if not user_id and not roles:
            auth_tags.add(f"[{self._PUBLIC_GROUP}:RW]")

        # Add user-specific permission
        if user_id:
            auth_tags.add(f"[USER_{user_id}:RW]")

        # Add role permissions
        if roles:
            for role in roles:
                if role and role != "admin":  # admin already added
                    auth_tags.add(f"[GRP_{role}:RW]")

        self.auths = auth_tags
        return self
    ...
    def add_auth(
        ...
        """Add a new auth permission to this UKF object (chainable).

        Adds a single permission pair to the existing auths without overwriting.
        The user_id should already be in the correct format (e.g., "USER_alice"
        or "GRP_analyst").

        Args:
            user_id: The auth identifier (e.g., "USER_alice", "GRP_analyst").
            authority: Permission level, either "RW" or "R".

        Returns:
            Self for method chaining.

        Raises:
            ValueError: If authority is not "RW" or "R".

        Examples:
            >>> ukf.add_auth("USER_bob", "R")  # Add read access for bob
            >>> ukf.add_auth("GRP_viewers", "R")  # Add read access for viewers group
            >>> ukf.auth(user_id="alice").add_auth("USER_bob", "R")  # Chainable
        """
        if authority not in self._VALID_AUTHORITIES:
            raise ValueError(f"Invalid authority: '{authority}'. Must be one of {self._VALID_AUTHORITIES}")

        # Get existing auths or initialize empty set
        existing_auths = set(self.auths or set())

        # Generate new auth tag and merge
        new_tag = f"[{user_id}:{authority}]"
        existing_auths.add(new_tag)

        self.auths = existing_auths
        return self
    ...
    def del_auth(
        ...
        """Remove an auth permission from this UKF object.

        Removes only if exact match is found (both user_id and authority must match).
        Cannot remove GRP_admin:RW as it is required for all UKFs.

        Args:
            user_id: The auth identifier to remove (e.g., "USER_alice", "GRP_analyst").
            authority: The authority level to remove ("RW" or "R").

        Returns:
            True if the permission was found and removed, False otherwise.

        Raises:
            ValueError: If attempting to remove GRP_admin:RW.

        Examples:
            >>> ukf.del_auth("USER_bob", "R")  # Remove bob's read access
            >>> ukf.del_auth("GRP_viewers", "R")  # Remove viewers' read access
        """
        # Prevent removal of admin group
        if user_id == self._ADMIN_GROUP and authority == "RW":
            raise ValueError(f"Cannot remove {self._ADMIN_GROUP}:RW - admin must retain access to all UKFs")

        # Get existing auths
        existing_auths = set(self.auths or set())

        # Build the exact tag to remove
        tag_to_remove = f"[{user_id}:{authority}]"

        if tag_to_remove in existing_auths:
            existing_auths.remove(tag_to_remove)
            self.auths = existing_auths
            return True
    ...
    def signed(self, system: bool = False, verified: bool = True, **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional provenance fields to override.
                ...
                - creator: set to the provided creator, or "system" if system is True, or existing creator, or current user id from CM_AHVN if existing creator is "unknown"
                - owner: set to the provided owner, or "system" if system is True, or existing owner, or current user id from CM_AHVN if existing owner is "unknown"
        ...
        updated_kwargs = {k: v for k, v in kwargs.items() if k not in ["source", "creator", "owner", "workspace", "inactive_mark", "last_verified"]} | {
            ...
            "creator": kwargs.get("creator", "system" if system else (self.creator if self.creator != "unknown" else CM_AHVN.get("user.user_id", "admin"))),
            "owner": kwargs.get("owner", "system" if system else (self.owner if self.owner != "unknown" else CM_AHVN.get("user.user_id", "admin"))),
