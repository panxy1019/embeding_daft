...
def instance_prompt_composer(kl, lang=None, **kwargs):
    """\
    Compose experience prompt text from PromptSpec registry.
    ...
    Args:
        ...
        lang (str, optional): Prompt rendering language.
    ...
    Example:
        ...
        - 25
    """
    prompt = PM_AHVN.get("experience_prompt")
    if not isinstance(prompt, PromptSpec):
        setup_system_prompts(force=False)
        prompt = PM_AHVN.get("experience_prompt")
    if not isinstance(prompt, PromptSpec):
        raise RuntimeError("Failed to load 'experience_prompt' from PM_AHVN.")
    return prompt(instance=kl.content_resources | kwargs, lang=lang)
