...
RESOURCE_UNZIP_TEMP_PATH = CM_AHVN.pj(CM_AHVN.get("core.tmp_path", tempfile.gettempdir()), "resource_unzip")
...
class _ResourceTempContext:
    ...
    def __enter__(self):
        if self._path is not None:
            self._tmp_path = CM_AHVN.pj(self._path)
        else:
            identifier = fmt_hash(md5hash(self._resource.content_hash_str, salt=self._resource.id_str))
            self._tmp_path = CM_AHVN.pj(RESOURCE_UNZIP_TEMP_PATH, identifier)
...
class ResourceUKFT(BaseUKF):
    ...
    def from_data(cls, data: Dict[str, Any], name: str = None, path: str = None, **updates):
        ...
        Example:
            ...
            "my_resource"
        """
        path = None if path is None else CM_AHVN.pj(path)
    ...
    def _temp_context(self, path: Optional[str] = None, overwrite: bool = False, cleanup: bool = False) -> _ResourceTempContext:
        if not path:
            resource_path = self.get("path")
            path = CM_AHVN.pj(resource_path, abs=True) if resource_path else None
