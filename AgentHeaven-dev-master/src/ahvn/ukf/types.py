...
def _get_text_lengths():
    return {"id": 63, "short": 255, "medium": 2047, "long": 65535} | CM_AHVN.get("ukf.text", {})
...
@_ukf_type
class UKFSetType(set):
    """Custom type for set with validation.

    Validates and converts various iterable types to a set of string.

    Args:
        value: Set, list, tuple, or other iterable of string values.
    ...
    Examples:
        >>> UKFSetType._validate(["synonym1", "synonym2"])
        {'synonym1', 'synonym2'}
        >>> UKFSetType._validate(None)
        set()
    """
