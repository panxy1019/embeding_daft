...
from ..utils.basic.str_utils import is_delimiter, is_spacy_available, normalize_text, indexed_normalize_text, resolve_match_conflicts
...
import sys
from typing import Any, Dict, List, Optional, Iterable, Literal, Callable, Union, Type, Tuple
...
class DAACKLEngine(BaseKLEngine):
        ...
        **kwargs,
        ...
        if encoder is None:
            ...
            def default_encoder(kl: BaseUKF) -> List[str]:
                return [kl.name] + (list(kl.synonyms) if kl.synonyms is not None else [])
        ...
        if (normalizer is None) or isinstance(normalizer, bool):
            ...
            if normalizer and is_spacy_available():
                ...
                self.normalizer = vanilla_normalizer
                self.indexed_normalizer = None
        else:
    ...
    def _search(
        ...
        char_origins: Optional[List[Tuple[int, int]]] = None,
        *args,
        **kwargs,
    ) -> List[Dict[str, Any]]:
        """\
    ...
    def _upsert(self, kl: BaseUKF, flush: bool = True, **kwargs):
        ...
        Args:
            ...
            kwargs: Additional keyword arguments.
        """
        # TODO: When update a BaseUKF object, reduced synonyms are not deleted
        # and will persist until the BaseUKF object is removed or the clear()
        # function is called
        ...
        if kl.normalized_synonyms:
            normalized_strings_to_add = list(kl.normalized_synonyms)
        else:
            normalized_strings_to_add = [self.normalizer(string) for string in strings_to_add]

        for normalized_string in normalized_strings_to_add:
            if len(normalized_string) < self.min_length:
                continue
    ...
    def _rebuild_automaton(self):
        ...
        if len(self.synonyms) > 0:
            for syn, kids in self.synonyms.items():
                ...
                for kid in kids:
                    rebuild_ac.add_word(inv_syn, inv_syn)

            rebuild_ac.make_automaton()
        else:
            rebuild_ac.make_automaton()
        self.ac = rebuild_ac
    ...
    def save(self, path: str = None):
        ...
        self.ac_load_time = get_mtime(pj(path, "ac.pkl"))
    ...
    def load(self, path: str = None) -> bool:
        ...
        ac_load_time = get_mtime(pj(path, "ac.pkl"))
        if self.ac_load_time == ac_load_time:
            return False
        ...
        self.ac_load_time = ac_load_time
        self.lazy_deletion = set()

        return True

    def __sizeof__(self) -> int:
        return sys.getsizeof(self.ac) + get_total_deepsize_of(self.synonyms, self.kl_synonyms, self.lazy_deletion)

    def getsizeof(self) -> int:
        return self.__sizeof__()

    def __lt__(self, other) -> bool:
        return len(self.synonyms) < len(other.synonyms)

    def synonyms_size(self) -> int:
