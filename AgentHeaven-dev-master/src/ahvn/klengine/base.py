...
class BaseKLEngine(ABC):
    ...
    def full_condition(self, kl: BaseUKF) -> bool:
        """\
        Check if a KL satisfies the engine's condition and the attached storage's condition (if any).

        Args:
            kl (BaseUKF): The KL to check.
        """
        if not self.condition(kl):
            return False
        if (self.storage is not None) and (self.storage.condition is not None):
            return self.storage.condition(kl)
        return True
    ...
    def batch_upsert(self, kls: Iterable[BaseUKF], progress: Type[Progress] = None, **kwargs):
        ...
        Args:
            ...
            kwargs: Additional keyword arguments.
        """
        filtered = [kl for kl in kls if self.full_condition(kl)]
        total = len(filtered)
    ...
    def remove(self, key: Union[int, str, BaseUKF], conditioned: bool = True, **kwargs):
        ...
        """
        if isinstance(key, BaseUKF):
            if conditioned and (not self.full_condition(key)):
                return
            key = key.id
        if isinstance(key, str):
            key = int(key)
