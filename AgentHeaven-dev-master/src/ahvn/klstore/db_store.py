...
class DatabaseKLStore(BaseKLStore):
    ...
    def __init__(self, database: Optional[str] = None, name: Optional[str] = None, condition: Optional[Callable] = None, *args, **kwargs):
        ...
        provider = kwargs.get("provider") or CM_AHVN.get("db.default_provider")
        database = database or CM_AHVN.get(f"db.providers.{provider}.database")
    ...
    def _init(self):
        for dim in self.adapter.dims.values():
            dim.__table__.create(self.db.engine, checkfirst=True)
