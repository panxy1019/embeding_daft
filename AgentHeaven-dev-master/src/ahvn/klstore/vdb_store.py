...
from ..ukf.base import BaseUKF, ptags
...
class VectorKLStore(BaseKLStore):
    ...
    def __init__(self, collection: Optional[str] = None, name: Optional[str] = None, condition: Optional[Callable] = None, *args, **kwargs):
        ...
        provider = kwargs.get("provider") or CM_AHVN.get("vdb.default_provider", "lancedb")
        encoder = kwargs.get("encoder")
        embedder = kwargs.get("embedder")
        include = kwargs.get("include")
        exclude = kwargs.get("exclude")
        collection = collection or kwargs.get("collection") or CM_AHVN.get(f"vdb.providers.{provider}.collection")
        connection_args = {
            k: v
    ...
    def _init(self):
        ...
        # UUID is used to avoid collisions when parallel instances are running
        dummy = DummyUKFT(name="<dummy>", content="This is a dummy node to initialize the collection.", tags=ptags(UUID=str(uuid4())))
