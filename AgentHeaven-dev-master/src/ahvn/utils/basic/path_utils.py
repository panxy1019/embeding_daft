...
def pj(*args: List[str], abs: bool = False, aliases: Optional[Dict[str, str]] = None) -> str:
    """\
    ...
    Args:
        *args: Components of the path to join. Each argument should be a string.
        ...
        aliases (Optional[Dict[str, str]], optional): A dictionary of prefix aliases to replace in the path.
            Each key will be replaced by its corresponding value in the final path. Defaults to None.
            ".", "..", "~", "$" are not recommended to be used as prefixes in aliases.
            Only prefixes in the first argument will be replaced.

    Returns:
        str: The joined, normalized path. Expands '~' to the user's home directory.
    """
    # Normalise each arg: strip spaces and trailing path separators, drop empties.
    args = [arg.strip().rstrip("/\\") for arg in args if arg.strip().rstrip("/\\") if args is not None]
    if not args:
        return os.sep
    if aliases:
        for prefix, replacement in aliases.items():
            if args[0].startswith(prefix):
                # Strip the prefix and any leading separators from the remainder so
                # that "&/foo" and "&foo" both append "foo" to the replacement rather
                # than letting os.path.join treat "/foo" as an absolute path.
                remainder = args[0][len(prefix) :].lstrip("/\\")
                new_first = [replacement] + ([remainder] if remainder else [])
                args = new_first + list(args[1:])
                break
    parts = [arg for arg in args if arg]
    path = os.path.expanduser(os.path.join(*parts) if parts else os.sep)
