...
from hashlib import md5 as MD5, sha256 as SHA256
...
def _serialize(obj):
    ...
    if isinstance(obj, (str, int, float, bool)):
        return obj  # Speedup for basic types, notice that this can be different from JSON serialization
    if isinstance(obj, dict) and not obj:  # Empty dict
        return "{}"
    if isinstance(obj, list) and not obj:  # Empty list
        return "[]"
    ...
    try:
        ...
        logger.warning(f"Failed to JSON serialize object {obj}: {e}. Falling back to hash() and repr().")
...
def sha256hash(obj: Any, salt: Any = None, sep: str = "||") -> str:
    """\
    Generate a SHA-256 hex digest for the given object.

    Unlike ``md5hash`` which returns an integer, this returns the
    full 64-character hex string — suitable for integrity verification.
    ...
    Returns:
        str: The SHA-256 hex digest (64 characters).
    """
    obj_repr, salt_repr = _serialize(obj), _serialize(salt)
    data = f"{obj_repr}" if salt is None else f"{obj_repr}{sep}{salt_repr}"
