...
def value_match(
    ...
    Args:
        ...
        case_sensitive (bool): Whether the matching should be case-sensitive. Defaults to True.
...
def raise_mismatch(
    ...
    Args:
        ...
        case_sensitive (bool): Whether the matching should be case-sensitive. Defaults to True.
        normalizer (Optional[callable]): An optional function to preprocess values before matching (e.g., for normalization). Defaults to None.
    ...
    if normalizer:
        mapping = {normalizer(s): s for s in supported}
        matches = value_match(list(mapping.keys()), normalizer(got), thres=thres, case_sensitive=case_sensitive)
        matches = [(mapping[m[0]], m[1]) for m in matches]
    else:
        matches = value_match(supported, got, thres=thres, case_sensitive=case_sensitive)
