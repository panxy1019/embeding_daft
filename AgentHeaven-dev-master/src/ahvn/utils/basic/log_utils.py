...
def encrypt_display(
    config: Dict[str, Any],
    encrypt_keys: Optional[List[str]] = None,
    show_front: int = 4,
    show_end: int = 2,
    custom_length: Optional[int] = 12,
    min_length: int = 10,
) -> Dict[str, Any]:
    """\
    Mask sensitive values in a config dictionary for display purposes.

    Each value whose key is in ``encrypt_keys`` is replaced with a masked
    string built from three parts: a visible prefix, a run of asterisks, and
    a visible suffix.  The exact shape depends on the options below.

    Args:
        config (Dict[str, Any]): The configuration dictionary to mask.
        encrypt_keys (Optional[List[str]]): Keys to mask.  If ``None``, reads
            ``core.encrypt_keys`` from the global AgentHeaven config.
        show_front (int): Number of original characters to show at the start.
            Defaults to 4.
        show_end (int): Number of original characters to show at the end.
            Defaults to 2.
        custom_length (Optional[int]): If set, the *total* length of the masked
            output (``show_front + asterisks + show_end``).  The asterisk
            count becomes ``custom_length - show_front - show_end`` (at least 1).
            If ``None`` / ``False``, the number of asterisks equals the number
            of hidden characters in the original value, keeping the output the
            same length as the input.  Defaults to 10.
        min_length (int): When the original value is shorter than this threshold
            the entire value is replaced with ``min_length`` asterisks (no
            front/end characters are revealed).  Defaults to 10.

    Returns:
        Dict[str, Any]: A deep-copied dictionary with masked values.

    Examples:
        >>> encrypt_cli({'api_key': 'supersecretpassword'}, encrypt_keys=['api_key'])
        {'api_key': 'supe****rd'}
        >>> encrypt_cli({'api_key': 'short'}, encrypt_keys=['api_key'])
        {'api_key': '**********'}
        >>> encrypt_cli({'api_key': 'mysecretpassword'}, encrypt_keys=['api_key'], custom_length=None)
        {'api_key': 'myse**********rd'}
    """
    from copy import deepcopy
    from ahvn.utils.basic.config_utils import CM_AHVN

    _encrypt_keys: set = set(CM_AHVN.get("core.encrypt_keys", [])) if encrypt_keys is None else set(encrypt_keys)
    config = deepcopy(config)
    for k, v in config.items():
        if k not in _encrypt_keys or not isinstance(v, str):
            continue
        if len(v) < min_length:
            config[k] = "*" * min_length
        else:
            front = v[:show_front]
            end = v[-show_end:] if show_end > 0 else ""
            if custom_length:
                num_stars = max(1, custom_length - show_front - show_end)
            else:
                num_stars = max(1, len(v) - show_front - show_end)
            config[k] = front + "*" * num_stars + end
    return config
