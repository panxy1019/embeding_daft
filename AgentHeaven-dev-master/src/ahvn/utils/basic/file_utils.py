...
def get_mtime(path: str) -> int:
    """\
    Return the time of last modification of the specified path.

    Args:
        path (str): The path to get.

    Returns:
        int: The mtime if the path exists and accessible, 0 otherwise.
    """
    path = CM_AHVN.pj(path, abs=True)
    try:
...
def list_dirs(path: str, abs: bool = False, reverse: bool = False) -> Generator[str, None, None]:
    """\
    ...
    Args:
        ...
        abs (bool, optional): If True, return absolute paths. Defaults to False.
        reverse (bool, optional): If True, return paths in descending order. Defaults to False.
    ...
    for entry in os.scandir(path):
        if entry.is_dir():
            entries.append(os.path.abspath(entry.path) if abs else os.path.relpath(entry.path, path))
    yield from sorted(entries, key=lambda name: name.lower(), reverse=reverse)
    return
...
def copy_file(src: str, dst: str, mode: Literal["replace", "skip", "strict"] = "replace") -> str:
    ...
    """
    src = CM_AHVN.pj(src, abs=True)
    dst = CM_AHVN.pj(dst, abs=True)
...
def copy_dir(src: str, dst: str, mode: Literal["replace", "skip", "strict", "merge"] = "merge", **kwargs) -> str:
    ...
    if mode == "strict":
        ...
        for f in enum_paths(src, abs=True):
            dstf = CM_AHVN.pj(dst, os.path.relpath(f, src), abs=True)
            if exists_path(dstf):
...
def folder_diagram(
    ...
    def build_recursive(current_path: str, current_prefix: str = "") -> List[str]:
        ...
        for index, entry in enumerate(dirs + files):
            ...
            rel_root_normalized = rel_root.replace(os.sep, "/")
            annotation_key = annotations.get(rel_root_normalized) or annotations.get(rel_root) or annotations.get(child_name)
