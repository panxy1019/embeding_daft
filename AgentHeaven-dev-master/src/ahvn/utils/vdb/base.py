...
class VectorDatabase(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.proxy = NetworkProxy(
            ...
            no_proxy=self.config.pop("no_proxy", None),
        )
    ...
    def connect(self) -> VectorStore:
        ...
        if self.backend == "pgvector":
            ...
            from ..db.spec import DatabaseConfigEngine, _build_url
            from ..db.db_utils import DatabaseEngineRegistry
            ...
            spec = engine.resolve(db_kwargs)
            engine_kw = engine.materialize(spec, mode="engine")
            connection_string = engine_kw.get("url", _build_url(spec))
            pg_config["connection_string"] = connection_string

            # Create both sync and async engines to satisfy PGVectorStore requirements
            sync_engine = DatabaseEngineRegistry.get_engine(spec)
