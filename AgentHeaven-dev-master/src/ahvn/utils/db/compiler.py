...
class SQLCompiler:
    ...
    def _parse_nf(orms: Dict[str, Any], entity: str, nf_entity: str, nf: Dict[str, Any]) -> ClauseElement:
        ...
        return sa.exists(
            sa.select(sa.distinct(orms[nf_entity].id)).where(
    ...
    def _parse(
        ...
        try:
            if op in ("AND", "OR"):
                ...
                if len(exprs) == 1:
                    return exprs[0]
                return sa.and_(*exprs) if op == "AND" else sa.or_(*exprs)

            if op == "NOT":
                return sa.not_(SQLCompiler._parse(orms=orms, entity=entity, field=field, expr=val))
