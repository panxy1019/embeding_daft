...
class ExportableEntity(get_base()):
    ...
    def _create_stmt(cls):
        return sa_schema.CreateTable(cls.__table__, if_not_exists=True)

    @classmethod
    def _drop_stmt(cls):
        return sa_schema.DropTable(cls.__table__, if_exists=True)
    ...
    def _exists_stmt(cls, record_id):
        return sa.select(sa.select(cls.id).where(cls.id == record_id).exists())

    @classmethod
    def _get_stmt(cls, record_id):
        return sa.select(cls).where(cls.id == record_id)
    ...
    def _insert_stmt(self, allow_skip, condition=None):
        entity_state = sa_orm.attributes.instance_state(self)
        values_dict = {c.key: getattr(self, c.key) for c in self.__table__.columns if c.key in entity_state.attrs}
        if allow_skip:
            condition = condition or ~sa.select(self.__class__.id).where(self.__class__.id == self.id).exists()
            binds = [sa.bindparam(k, v, type_=self.__table__.columns[k].type) for k, v in values_dict.items()]
            select_stmt = sa.select(*binds).where(condition)
            return sa.insert(self.__class__).from_select(names=list(values_dict.keys()), select=select_stmt)
        return sa.insert(self.__class__).values(**values_dict)
...
class DatabaseIdType(sa_types.TypeDecorator):
    """\
    ...
    def load_dialect_impl(self, dialect):
        return sa_types.String(CM_AHVN.get("ukf.text.id", 63))
...
class DatabaseTextType(sa_types.TypeDecorator):
    """\
    ...
    def load_dialect_impl(self, dialect):
        if dialect.name == "mysql" and self.length and self.length >= 16384:
            return sa_mysql.TEXT()
        return sa_types.String(self.length)
...
class DatabaseIntegerType(sa_types.TypeDecorator):
    """\
...
class DatabaseBooleanType(sa_types.TypeDecorator):
    """\
    Custom Boolean type for database models.
    """

    impl = sa_types.Boolean
    cache_ok = True
...
class DatabaseDurationType(sa_types.TypeDecorator):
    """\
...
class DatabaseJsonType(sa_types.TypeDecorator):
    """\
    ...
    def _get_native_dialects(self):
        return {
            "postgresql": sa_pg.JSONB(),
            "bigquery": sa_types.JSON(),
            "snowflake": sa_types.JSON(),
            "trino": sa_types.JSON(),
            "duckdb": sa_types.JSON(),
            "databricks": sa_types.JSON(),
            "spark": sa_types.JSON(),
            "presto": sa_types.JSON(),
        }

    def _get_string_dialects(self):
        return {
            "sqlite": sa_types.TEXT(),
            "oracle": sa_oracle.CLOB(),
            "starrocks": sa_types.TEXT(),
            "hana": sa_oracle.NCLOB(),
            "hive": sa_types.TEXT(),
    ...
    def load_dialect_impl(self, dialect):
        ...
        return dialect.type_descriptor(sa_types.String(CM_AHVN.get("ukf.text.long", 65535)))
...
class DatabaseSetType(sa_types.TypeDecorator):
    """\
    Custom Set type for database models.
    """
...
class DatabaseTimestampType(sa_types.TypeDecorator):
    """
    ...
    def _get_native_dialects(self):
        return {
            "postgresql": sa_types.TIMESTAMP(timezone=True),
            "mssql": sa_types.DATETIME(),
            "oracle": sa_types.TIMESTAMP(),
            "snowflake": sa_types.TIMESTAMP(timezone=True),
            "bigquery": sa_types.TIMESTAMP(),
            "duckdb": sa_types.TIMESTAMP(),
            "trino": sa_types.TIMESTAMP(),
            "databricks": sa_types.TIMESTAMP(),
            "spark": sa_types.TIMESTAMP(),
        }

    def _get_integer_dialects(self):
        return {
            "sqlite": sa_types.BigInteger(),
            "starrocks": sa_types.BigInteger(),
            "hive": sa_types.BigInteger(),
            "presto": sa_types.BigInteger(),
            "hana": sa_types.BigInteger(),
    ...
    def load_dialect_impl(self, dialect):
        ...
        return dialect.type_descriptor(sa_types.BigInteger())
...
class DatabaseNfType(sa_types.TypeDecorator):
    """
...
class DatabaseVectorType(sa_types.TypeDecorator):
    ...
    def _native_dialects(self):
        return {
            "postgresql": sa.ARRAY(sa_types.Float()),
        }
    ...
    def process_bind_param(self, ukf_value, dialect):
        ...
        return DatabaseJsonType().process_bind_param([float(item) for item in ukf_value], dialect)
