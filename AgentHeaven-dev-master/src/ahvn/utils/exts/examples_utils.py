...
from typing import Iterable, Union, Dict, Any, Optional, Generator, Callable
...
def normalize_examples(
    ...
    Args:
        ...
        search_encoder: Optional function to customize search behavior for KLEngine and KLBase inputs
            It should take a CacheEntry (instance) as input and return a dictionary of search arguments.
        instance: Optional CacheEntry instance to be passed to search_encoder
    ...
    Examples:
        ...
        >>> list(normalize_examples(engine, search_encoder=lambda instance: {"topk": 5, "type": "experience"}))
        [CacheEntry(...), ...]
        ...
        >>> list(normalize_examples(klbase, search_encoder=lambda instance: {"topk": 5, "type": "experience"}))
        [CacheEntry(...), ...]
    """
    if examples is None:
        yield from []
    ...
    if isinstance(examples, BaseKLStore):
        ...
        yield from normalize_examples(engine, search_encoder=search_encoder, instance=instance)
        return

    # If it's a KLEngine or KLBase, use search method
    if isinstance(examples, BaseKLEngine) or isinstance(examples, KLBase):
        search_args = search_encoder(instance) if search_encoder is not None else dict()
