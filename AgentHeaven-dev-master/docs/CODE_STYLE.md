# AgentHeaven Code Style Guide

> Mandatory style guide for **every** engineer — human or AI — writing, reviewing,
> or auto-generating code in any project built on top of AgentHeaven (AHVN):
> `AgentHeaven`, `RubikSQL`, `RubikSQL-gui`, `RubikBench`, tutorials, skills,
> experiments, and internal tools.
>
> Official package documentation: <https://ahvn.top/>.
>
...
| `import tempfile`, bespoke tmp dirs    | `ahvn.utils.basic.file_utils` (scratch dirs resolved via `CM_AHVN.pj`) |
...
**Rule of thumb:** before `import <stdlib>`, check `src/ahvn/utils/basic/`
for a `*_utils.py` with the same concern. If one exists, use it. If one does
not exist *and* the stdlib call is non-trivial, add a helper there rather than
sprinkling raw stdlib across the codebase.
...
p = CM_AHVN.pj("&/cache/x.json", abs=True)   # & = CM root, resolves aliases
```
...
Each package exposes exactly one config manager:
...
2. **No hard-coded example strings or prompt fragments** in `.py` files.
   ...
   are loaded via the config manager (see the online docs for the resource
   registry).
3. **No hard-coded paths.** Every path goes through `CM_*.pj(...)` with
   aliases; never concatenate raw strings.
4. **No hard-coded model / provider / table / index names.** All naming
   lives in config or a typed registry.
5. **Changing the default config template** (e.g. `secret/default_config.yaml`)
   requires running the repo's setup script (e.g. `bash secret/setup.bash`)
   so the local user config is regenerated.

For detailed usage of the config system, see
<https://ahvn.top/> → *Configuration*.
...
- **One configuration schema at a time.** When the schema changes, bump the
  default config file and update `secret/setup.bash` (or the equivalent).
...
For translations (EN ↔ ZH), consult `AgentHeaven-docs/i18n.md` before
introducing a new term. If you invent an abbreviation, add it here in the
same PR.
...
- **No `print` for diagnostics.** Use `ahvn.utils.basic.log_utils`. Debug
  output is gated on `CM_AHVN.get("core.debug")`.
...
- **Never invoke `pytest` directly.** Always use the repo's test script
  (e.g. `bash scripts/test.bash ...`) so environment, markers, and coverage
  are consistent.
...
- Bilingual docs (EN / ZH) follow `AgentHeaven-docs/i18n.md` terminology.

---

## 13. Tooling and Pre-commit

- **`black` + `flake8` must pass** before every commit. Use the repo's
  formatter script (e.g. `bash scripts/flake.bash -b -f`). CI enforces both.
...
- Every PR description states:
  ...
  3. Any `CM_*` default-config updates needed (run the setup script).
...
- [ ] `black` + `flake8` clean.
- [ ] Tests run via the repo's test script and pass.
