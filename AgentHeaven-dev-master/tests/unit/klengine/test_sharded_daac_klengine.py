"""
Comprehensive tests for ShardedDAACKLEngine functionality.

This module tests the ShardedDAACKLEngine class which provides AC automaton-based string search.
The DAAC engine is always non-inplace and non-recoverable, only returning IDs by default.

The tests cover all ShardedDAACKLEngine functionality including:
...
from ahvn.klengine.sharded_daac_engine import ShardedDAACKLEngine
...
class TestShardedDAACKLEngine:
    """Test ShardedDAACKLEngine functionality."""
    ...
    def create_test_ukfs(self) -> List[BaseUKF]:
        ...
        ukfs = [
            ...
            BaseUKF(
                ...
                type="field",
                content="Reinforcement learning is a subset of ML",
                synonyms=["rl", "reinforcement learning", "RL"],
            ),
        ]
        return ukfs
    ...
    def daac_engine_cache(self, populated_cache_store, temp_dir):
        """Create a Sharded DAAC engine with cache storage."""

        def encoder(kl):
            return [kl.name] + list(kl.synonyms if kl.synonyms else [])

        engine = ShardedDAACKLEngine(
    ...
    @pytest.fixture
    def daac_engine_cache_multi_shards(self, populated_cache_store, temp_dir):
        """Create a Sharded DAAC engine with cache storage."""
        ...
        engine = ShardedDAACKLEngine(
            storage=populated_cache_store,
            path=str(temp_dir / "daac_cache_multi_shards"),
            encoder=encoder,
            min_length=2,
            inverse=True,
            normalizer=True,
    ...
    @pytest.fixture
    def daac_engine_cache_origin(self, populated_cache_store, temp_dir):
        ...
        engine = DAACKLEngine(
            storage=populated_cache_store,
            path=str(temp_dir / "daac_cache_inner"),
            encoder=encoder,
            min_length=2,
            inverse=True,
            normalizer=True,
    ...
    def daac_engine_db(self, populated_db_store, temp_dir):
        """Create a Sharded DAAC engine with database storage."""
    ...
    def test_initialization_basic(self, populated_cache_store, temp_dir):
        ...
        assert engine.max_shard_size == 10000
        assert len(engine.shards) == 0
        assert engine.next_idx == 0
    ...
    def test_search_multi_shards(self, daac_engine_cache_multi_shards):
        """Test basic search on multi-shards."""
        results = daac_engine_cache_multi_shards.search(query="I'm learning python and rl", include=["id"])
        assert len(daac_engine_cache_multi_shards.shards) > 1
    ...
    def test_search_conflict_overlap_compare(self, daac_engine_cache_origin, daac_engine_cache_multi_shards):
        """Test search with overlap conflict strategy on multi-shards."""
        results1 = daac_engine_cache_origin.search(query="I use javascript daily", conflict="overlap", include=["id", "matches", "strs"])
        results2 = daac_engine_cache_multi_shards.search(query="I use javascript daily", conflict="overlap", include=["id", "matches", "strs"])
        java = next(kl for kl in daac_engine_cache_origin.storage if kl.name == "java")

        assert results1 == results2
        assert next(result for result in results1 if result["id"] == java.id)["matches"] == [(6, 10)]
        assert next(result for result in results1 if result["id"] == java.id)["strs"] == ["java"]

    def test_search_conflict_longest_distinct_compare(self, daac_engine_cache_origin, daac_engine_cache_multi_shards):
        """Test search with longest_distinct conflict strategy on multi-shards."""
        results1 = daac_engine_cache_origin.search(query="python javascript java", conflict="longest_distinct", include=["id", "matches"])
        results2 = daac_engine_cache_multi_shards.search(query="python javascript java", conflict="longest_distinct", include=["id", "matches"])
    ...
    def test_search_no_matches_multi_shards(self, daac_engine_cache_multi_shards):
        """Test search with query that has no matches."""
        results = daac_engine_cache_multi_shards.search(query="极其罕见的不存在的词语xyzqwerty999", include=["id"])
        assert len(results) == 0
    ...
    def test_remove_and_search_multi_shards(self, daac_engine_cache_multi_shards):
        """Test removing UKFs and verifying they're not in search results."""
        # Find a UKF to remove
        all_ukfs = list(daac_engine_cache_multi_shards.storage)
        assert len(all_ukfs) > 0

        ukf_to_remove = all_ukfs[0]

        # Search before removal
        results_before = daac_engine_cache_multi_shards.search(query=ukf_to_remove.name, include=["id"])

        # Remove from engine
        daac_engine_cache_multi_shards.remove(ukf_to_remove.id)
        daac_engine_cache_multi_shards.flush()

        # Search after removal
        results_after = daac_engine_cache_multi_shards.search(query=ukf_to_remove.name, include=["id"])
    ...
    def test_batch_remove_basic(self, daac_engine_cache):
        ...
        all_ukfs = list(daac_engine_cache.storage)
        assert len(all_ukfs) >= 6

        ukfs_to_remove = all_ukfs[:5]
        ukfs_to_keep = all_ukfs[5:]

        # Batch remove first 5
    ...
    def test_batch_remove_basic_multi_shards(self, daac_engine_cache_multi_shards):
        """Test basic batch_remove functionality."""
        all_ukfs = list(daac_engine_cache_multi_shards.storage)
        assert len(all_ukfs) >= 6
        ...
        # Batch remove first 5
        daac_engine_cache_multi_shards.batch_remove([ukf.id for ukf in ukfs_to_remove])
        daac_engine_cache_multi_shards.flush()

        # Verify removed
        for ukf in ukfs_to_remove:
            results = daac_engine_cache_multi_shards.search(query=ukf.name, include=["id"])
            ids = {r["id"] for r in results}
            assert ukf.id not in ids
    ...
    def test_sync_multi_shards(self, populated_cache_store, temp_dir):
        """Test batch upsert operations."""
        ...
        assert len(engine) == len(populated_cache_store)

        results = engine.search(query="I'm learning python and rl", include=["id"])
        assert len(results) == 2
    ...
    def test_save_and_load_multi_shards(self, populated_cache_store, temp_dir):
        """Test saving and loading engine state."""
        path = str(temp_dir / "test_save_load_multi_shards")
    ...
    def test_search_consistency(self, daac_engine_cache_multi_shards):
        ...
        results1 = daac_engine_cache_multi_shards.search(**search_params)
        results2 = daac_engine_cache_multi_shards.search(**search_params)
        results3 = daac_engine_cache_multi_shards.search(**search_params)
    ...
    def test_search_during_rebuild_concurrency(self, daac_engine_cache_multi_shards):
        """Test that search works while the automaton is being rebuilt."""
        initial_results = daac_engine_cache_multi_shards.search(query="python", include=["id"])
        ...
        with patch.object(daac_mod.ac, "Automaton", side_effect=DelayedAutomaton):
            ...
            results_during = daac_engine_cache_multi_shards.search(query="python", include=["id"])
            end_time = time.time()
        ...
        results_after = daac_engine_cache_multi_shards.search(query="python", include=["id"])
