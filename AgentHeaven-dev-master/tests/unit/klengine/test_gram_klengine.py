"""
Comprehensive tests for GRAMKLEngine functionality.

This module tests the GRAMKLEngine class which provides AC automaton-based string search.
The GRAM engine is always non-inplace and non-recoverable, only returning IDs by default.

The tests cover all GRAMKLEngine functionality including:
...
from ahvn.klengine.gram_engine import GRAMKLEngine
...
class TestGRAMKLEngine:
    """Test GRAMKLEngine functionality."""
    ...
    @pytest.fixture
    def temp_dir(self):
        """Create a temporary directory for GRAM engine files."""
    ...
    @pytest.fixture
    def gram_engine_cache(self, populated_cache_store, temp_dir):
        """Create a GRAM engine with cache storage."""
    ...
    @pytest.fixture
    def gram_engine_db(self, populated_db_store, temp_dir):
        """Create a GRAM engine with database storage."""
    ...
    def test_initialization_basic(self, populated_cache_store, temp_dir):
        """Test basic GRAM engine initialization."""
    ...
    def test_initialization_with_custom_parameters(self, populated_cache_store, temp_dir):
        """Test GRAM engine initialization with custom parameters."""
    ...
    def test_initialization_with_default_encoder(self, populated_cache_store, temp_dir):
        """Test GRAM engine with None encoder (should use default)."""
        engine = GRAMKLEngine(
            storage=populated_cache_store,
    ...
    def test_search_basic_id_only(self, gram_engine_cache):
        """Test basic search returning IDs only."""
        results = gram_engine_cache.search(query="I love python programming", include=["id"])
    ...
    def test_search_with_query_include(self, gram_engine_cache):
        """Test search including normalized query."""
        query = "I Love Python Programming"
        results = gram_engine_cache.search(query=query, include=["id", "query"])
    ...
    def test_search_with_matches_include(self, gram_engine_cache):
        """Test search including match positions."""
        results = gram_engine_cache.search(query="python and javascript", include=["id", "matches"])
    ...
    def test_search_with_kl_include(self, gram_engine_cache):
        """Test search including KL objects from storage."""
        results = gram_engine_cache.search(query="python programming", include=["id", "kl"])
    ...
    def test_search_with_all_includes(self, gram_engine_cache):
        """Test search with all include options."""
        results = gram_engine_cache.search(query="python and javascript", include=["id", "kl", "query", "matches"])
    ...
    def test_search_conflict_overlap(self, gram_engine_cache):
        """Test search with overlap conflict strategy."""
        # "javascript" contains "java" - both should match with overlap
        results = gram_engine_cache.search(query="I use javascript daily", conflict="overlap", include=["id", "matches"])

        # Should find multiple matches (javascript and possibly java if it overlaps)
        assert len(results) >= 1

    def test_search_conflict_longest_distinct(self, gram_engine_cache):
        """Test search with longest_distinct conflict strategy."""
        results = gram_engine_cache.search(query="python javascript java", conflict="longest_distinct", include=["id", "matches"])

        assert len(results) >= 1

    # === Search Tests - Whole Word Matching ===

    def test_search_whole_word_true(self, gram_engine_cache):
        ...
        gram_engine_cache.storage.upsert(ukf)
        gram_engine_cache.upsert(ukf)
        gram_engine_cache.flush()
        results_whole = gram_engine_cache.search(query="python py", whole_word=True, include=["id", "kl", "str", "matches"])

        results_partial = gram_engine_cache.search(query="python py", whole_word=False, include=["id", "kl", "str", "matches"])

        # Both should find matches since "python" is a whole word
        assert len(results_whole) >= 1
        assert len(results_partial) >= 1

    def test_search_whole_word_false(self, gram_engine_cache):
        ...
        results = gram_engine_cache.search(query="python programming", whole_word=False, include=["id", "kl", "str", "matches"])
    ...
    def test_search_empty_query(self, gram_engine_cache):
        """Test search with empty query."""
        results = gram_engine_cache.search(query="", include=["id"])
        assert len(results) == 0

    def test_search_no_matches(self, gram_engine_cache):
        """Test search with query that has no matches."""
        results = gram_engine_cache.search(query="极其罕见的不存在的词语xyzqwerty999", include=["id"])
        assert len(results) == 0

    def test_search_case_insensitive(self, gram_engine_cache):
        """Test that search is case-insensitive with normalizer."""
        results_lower = gram_engine_cache.search(query="python", include=["id"])
        results_upper = gram_engine_cache.search(query="PYTHON", include=["id"])
        results_mixed = gram_engine_cache.search(query="PyThOn", include=["id"])

        # All should return the same results due to normalization
        assert len(results_lower) == len(results_upper) == len(results_mixed)

    def test_search_with_multiple_matches_same_ukf(self, gram_engine_cache):
        """Test search where same UKF matches multiple times."""
        # "python" and "py" both refer to the same UKF
        results = gram_engine_cache.search(query="python is py python", include=["id", "matches"])
    ...
    def test_remove_and_search(self, gram_engine_cache):
        """Test removing UKFs and verifying they're not in search results."""
        # Find a UKF to remove
        all_ukfs = list(gram_engine_cache.storage)
        if len(all_ukfs) > 0:
            ukf_to_remove = all_ukfs[0]

            # Search before removal
            results_before = gram_engine_cache.search(query=ukf_to_remove.name, include=["id"])

            # Remove from engine
            gram_engine_cache.remove(ukf_to_remove.id)
            gram_engine_cache.flush()

            # Search after removal
            results_after = gram_engine_cache.search(query=ukf_to_remove.name, include=["id"])
    ...
    def test_clear(self, gram_engine_cache):
        """Test clearing all UKFs from engine."""
        assert len(gram_engine_cache) > 0

        gram_engine_cache.clear()

        assert len(gram_engine_cache) == 0

        results = gram_engine_cache.search(query="python", include=["id"])
        assert len(results) == 0
    ...
    def test_batch_remove_basic(self, gram_engine_cache):
        ...
        if len(all_ukfs) >= 5:
            ...
            gram_engine_cache.batch_remove([ukf.id for ukf in ukfs_to_remove])
            gram_engine_cache.flush()

            # Verify removed
            for ukf in ukfs_to_remove:
                results = gram_engine_cache.search(query=ukf.name, include=["id"])
    ...
    def test_batch_remove_with_ukf_instances(self, gram_engine_cache):
        ...
        if len(all_ukfs) >= 3:
            ...
            gram_engine_cache.batch_remove(ukfs_to_remove)
            gram_engine_cache.flush()
    ...
    def test_batch_remove_empty_list(self, gram_engine_cache):
        """Test batch_remove with empty list."""
        len_before = len(gram_engine_cache)

        # Remove empty list (should do nothing)
        gram_engine_cache.batch_remove([])
        gram_engine_cache.flush()

        # Length should be unchanged
        assert len(gram_engine_cache) == len_before
    ...
    def test_with_database_storage(self, gram_engine_db):
        """Test GRAM engine with database storage."""
        results = gram_engine_db.search(query="python programming", include=["id", "kl"])
    ...
    def test_with_cache_storage(self, gram_engine_cache):
        """Test GRAM engine with cache storage."""
        results = gram_engine_cache.search(query="javascript development", include=["id", "kl"])
    ...
    def test_double_remove(self, gram_engine_cache):
        """Test removing the same UKF twice."""
        all_ukfs = list(gram_engine_cache.storage)
        if len(all_ukfs) > 0:
            ukf = all_ukfs[0]

            gram_engine_cache.remove(ukf.id)  # First removal
            result2 = gram_engine_cache.remove(ukf.id)  # Second removal
    ...
    def test_search_consistency(self, gram_engine_cache):
        ...
        results1 = gram_engine_cache.search(**search_params)
        results2 = gram_engine_cache.search(**search_params)
        results3 = gram_engine_cache.search(**search_params)
